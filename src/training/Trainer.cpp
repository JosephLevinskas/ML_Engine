...
TrainingResults Trainer::train(const LinearModel& model,
                               const Matrix& X,
    ...
    trainingObjectiveLosses.reserve(epochs);
    ...
    for (size_t i = 0; i < epochs; ++i) {
        ...
        double objectiveLoss =
            dataLoss + l2Penalty(current.getWeights(), lambda);

        dataLosses.push_back(dataLoss);
        trainingObjectiveLosses.push_back(objectiveLoss);
        ...
        for (size_t j = 0; j < gradients.weightGradients.size(); ++j) {
            gradients.weightGradients[j] +=
                lambda * current.getWeights()[j];
        }

        Vector updatedWeights =
            current.getWeights() - learningRate * gradients.weightGradients;

        double updatedBias =
            current.getBias() - learningRate * gradients.biasGradient;
    ...
    return {
        std::move(current),
        std::move(dataLosses),
        std::move(trainingObjectiveLosses)
    };
}

LogisticTrainingResults Trainer::train(const LogisticModel& model,
                                       const Matrix& X,
                                       const Vector& targets) const {
